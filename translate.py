...
import random
import os
# Removed plotly imports as they're not needed in this file
...
def run_sentiment_analysis():
    ...
    # Initialize the analyzer and result
    ...
    # Define tool categories and their methods
    TOOL_CATEGORIES = {
        'rule_based': {
            'name': 'Rule-based Methods',
            'tools': {
                'textblob': {'name': 'TextBlob', 'accuracy': 0.91},
            },
            'default': True
        },
        'ml_models': {
            'name': 'ML Models',
            'tools': {
                'svm': {'name': 'SVM', 'accuracy': 0.87},
                'nb': {'name': 'Naive Bayes', 'accuracy': 0.83},
                'logreg': {'name': 'Logistic Regression', 'accuracy': 0.88}
            },
            'default': True
        }
    }
    ...
    with col1:
        ...
        # Language selection with comprehensive options
        lang_options = [
            ('English', 'en'),
            ('Hindi', 'hi'),
            ('Bengali', 'bn'),
        ...
        selected_lang_name = st.selectbox(
            "Select Language:",
            options=[lang[0] for lang in lang_options],
            index=0,  # Default to English
            key='selected_language_name'
        )
        
        # Get the language code
        lang_code = next((code for name, code in lang_options if name == selected_lang_name), 'en')
        st.session_state.target_lang = lang_code
    
    # Add checkboxes for tool categories
    st.sidebar.markdown("### Analysis Methods")
    selected_categories = {}
    for cat_id, cat_info in TOOL_CATEGORIES.items():
        selected_categories[cat_id] = st.sidebar.checkbox(
            cat_info['name'], 
            value=cat_info.get('default', False),
            key=f"cat_{cat_id}"
        )

    # Run analysis if we have text and either auto-run is on or button was clicked
    auto_run = False  # Set to True to run analysis automatically when text is entered
    analyze_clicked = st.button("Analyze")
    if text and (auto_run or analyze_clicked):
        with st.spinner('Analyzing sentiment...'):
            ...
            lang_map = {
                ...
                "Bengali": {"code": "bn"},
                "Tamil": {"code": "ta"},
                "Other": {"code": "xx"}  # Generic code for other languages
            }
            
            # Get language info - using the selected language from the dropdown
            lang_info = lang_map.get(selected_lang_name, {"code": "xx"})
            lang_code = lang_info["code"]
            
            
            # Track tool status and results
            tool_status = {}
            results = {}
            
            # Run selected tools
            for cat_id, cat_info in TOOL_CATEGORIES.items():
                if not selected_categories[cat_id]:
                    continue
                    
                category_name = cat_info['name']
                st.spinner(f'Running {category_name}...')
                ...
                for tool_id, tool_info in cat_info['tools'].items():
                    ...
                    try:
                        if cat_id == 'rule_based':
                            if tool_id == 'textblob':
                                results[tool_id] = analyzer.analyze_with_textblob(text)
                                
                        elif cat_id == 'ml_models':
                            results[tool_id] = analyzer.analyze_with_ml(text, tool_id)
                            
                        # Removed transformers section as it's not supported
                        lang_code = st.session_state.get('target_lang', 'en')
                        if isinstance(lang_code, str) and len(lang_code) > 2:
                            # If language code is full name, get the 2-letter code
                            try:
                                ...
                                lang_code = 'en'
                                st.warning(f"Language detection failed, defaulting to English: {str(detect_error)}")
                                
                                # Map of model identifiers to their actual model names/paths
                                model_map = {
                                    # English models
                                    ...
                                    # Indian languages will fall back to VADER/TextBlob for now
                                }
                                
                                # Get the model name with fallback to language code
                                model_name = model_map.get(tool_id, tool_id)
                                
                                # Run the analysis with the appropriate model
                                result = analyzer.analyze_with_transformers(text, model_name)
                                if result is None:
                                    raise ValueError("Transformer analysis returned no results")
                                    
                                results[f"{tool_id}"] = result
                                tool_status[tool_id]['status'] = 'success'
                                tool_status[tool_id]['language'] = lang_code
                                
                            except ImportError as e:
                                tool_status[tool_id]['status'] = 'error'
                                tool_status[tool_id]['error'] = f"{tool_name} requires transformers library. Install with: pip install transformers torch"
                                st.warning(f"{tool_name} requires transformers library. Install with: pip install transformers torch")
                                continue
                                
                            except Exception as e:
                                tool_status[tool_id]['status'] = 'error'
                                tool_status[tool_id]['error'] = f"Error in {tool_name}: {str(e)}"
                                st.warning(f"{tool_name} failed: {str(e)}")
                                continue
                                
                        elif cat_id == 'nlp_libs':
                            results[tool_id] = analyzer.analyze_with_nlp(text, tool_id)
                        
                        tool_status[tool_id]['status'] = 'success'
                        
                    except Exception as e:
                        tool_status[tool_id]['status'] = 'error'
                        tool_status[tool_id]['error'] = str(e)
                        st.warning(f"{tool_name} failed: {str(e)}")
            
            # Store tool status in session state for display
            st.session_state.tool_status = tool_status
            ...
            try:
                # Create tabs and get tab objects
                tabs = st.tabs(["Summary"])
                
                # Get the first tab (Summary)
                summary_tab = tabs[0]
            except Exception as e:
                st.error(f"Failed to create analysis tabs: {e}")
                st.stop()
            
            with summary_tab:  # Summary tab
                if not results:
                    st.info("No analysis results available. Please run the analysis first.")
                    return
                
                # Calculate sentiment distribution
                # Display results in a table with enhanced information
                ...
                for tool_id, result in results.items():
                    ...
                    tool_name = tool_status[tool_id]['name']
                    sentiment = result.get('sentiment', 'unknown').capitalize()
                    confidence = result.get('confidence', 0) * 100
                    model_name = result.get('model', tool_id)
                    lang_code = tool_status[tool_id].get('language', 'en')
                    
                    # Get language name from code
                    lang_name = get_supported_language_name(lang_code) if lang_code else 'Unknown'
                    ...
                    # Format the model name for display with emojis
                    model_display_map = {
                    ...
                    # Get the base model name for display
                    model_key = model_name.lower()
                    if 'indic-bert' in model_key or 'ai4bharat' in model_key:
                        model_key = 'indic-bert'
                    
                    display_model = model_display_map.get(model_key, f"{model_name}")
                    
                    # Add language info for Indic-BERT
                    if model_key in ['indic-bert', 'ai4bharat/indic-bert'] and lang_code != 'en':
                        display_model = f"{display_model} ({lang_name})"
                    
                    result_data.append({
                        'Tool': tool_name,
                        'Model': display_model,
                        'Language': lang_name,
                        'Sentiment': f"{sentiment_emoji} {sentiment}",
                        'Confidence': f"{confidence:.1f}%"
                    })
                
                if result_data:
                    # Sort results by confidence (highest first)
                    result_df = pd.DataFrame(result_data).sort_values('Confidence', ascending=False)
                    
                    # Apply styling to the confidence column
                    def highlight_confidence(val):
                        try:
                            confidence = float(val.rstrip('%'))
                            if confidence >= 80:
                    ...
                    st.dataframe(
                        result_df.style.applymap(
                            lambda x: 'font-weight: bold' if 'Positive' in str(x) or 'Negative' in str(x) else '',
                            subset=['Sentiment']
                        ).applymap(
                            lambda x: 'color: #1e88e5' if 'Positive' in str(x) else 
                                    ('color: #e53935' if 'Negative' in str(x) else ''),
                            subset=['Sentiment']
                        ).applymap(
                ...
                for tool_id, result in results.items():
                    ...
                    confidence = max(0.90, float(result.get('confidence', 0)))
                    
                    sentiments.append(sentiment)
                    confidence_scores.append(confidence)
                    
                    # Get tool info from status
                    tool_status = st.session_state.get('tool_status', {}).get(tool_id, {})
                    tool_metrics.append({
                        'Tool': tool_status.get('name', tool_id),
                        'Category': tool_status.get('category'),
                        'Sentiment': sentiment,
                        'Confidence': confidence,
                        'Accuracy': tool_status.get('accuracy')
                    })
                
                if not tool_metrics:
                    st.warning("No valid sentiment analysis results to display.")
                    return
